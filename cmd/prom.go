...
type GitLabCollector struct {
	...
	opts          *internal.GitlabScrapeOpts
}

type MetricsSender interface {
	SendProbeFailure(ch chan<- prometheus.Metric)
	SendMetrics(ch chan<- prometheus.Metric, metrics *internal.Metrics, opts *internal.GitlabScrapeOpts)
}
...
func (c *GitLabCollector) Collect(ch chan<- prometheus.Metric) {
	metrics, err := internal.GetMetrics(c.client, c.opts)
